...
std::vector<Token> Lex(const char *fileName)
{
    ...
    do {
        fp.get(c);
        if (!isvalidBF(c) && fp.good()) continue;
...
// Optimizes very common simple brainfuck algorithms
std::vector<Token> Optimize(const std::vector<Token> &v)
{
    std::vector<Token> r1, r2;
    bool hc = false;
    for (size_t i = 0; i < v.size(); ++i) {
        ...
        if (i < v.size() - 3 && v.at(i).c == '[' &&
            ...
            if (hc) r1.push_back({'e', 1});
            i += 2;
        }
        else r1.push_back(v.at(i));
        if (v.at(i).c == 'n' || v.at(i).c == ',') hc = true;
    }
    return r1;
    // TODO: implement a second pass to merge the previously disjointed
    //* (by the no longer existing 'e') instructs
    // for (size_t i = 0; i < r1.size(); ++i) {
    //     if (i > 0 && r1.at(i).c == r1.at(i - 1).c)
    //     {
    //         while () {}
    //         r2.push_back({r1.at(i).c, r1.at(i).amount + r1.at(i -
    //         1).amount});
    //     }
    //     else r2.push_back(r1.at(i));
    // }
    // return r2;
    // TODO: implement 'a' inst for absolute movement
}
...
AST::~AST() {
} //! DO NOT REMOVE, this is used so that no linker vtable error occur
...
class StmtAST : public AST {
    ...
    public:
    StmtAST(std::vector<std::unique_ptr<AST>> &&vptr)
        : children(std::move(vptr))
    {
    }
...
std::unique_ptr<AST> ParseExpression(std::vector<Token>::const_iterator &nit)
{
    ...
    if (nit == tokens.cend()) t.push_back(Token::None);
...
ParseTopStatement(std::vector<Token>::const_iterator &nit)
{
    std::vector<std::unique_ptr<AST>> r;
    do {
        if (auto e = ParseExpression(nit); e != nullptr)
